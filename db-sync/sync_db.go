...
func synchronize() {
	primaryDB, err := sql.Open("mysql", "root:root@tcp(mysql:3306)/camp_esprit")
	if err != nil {
		log.Fatalf("Failed to connect to primary database: %v", err)
	}
	defer primaryDB.Close()

	secondaryDB, err := sql.Open("mysql", "root:root@tcp(mysql-backend:3306)/camp_esprit")
	if err != nil {
	...
	fmt.Println("Synchronization complete.", time.Now())
}

func main() {
	c := cron.New(cron.WithSeconds())
	_, err := c.AddFunc("0 * * * * *", synchronize)
	if err != nil {
		return
	} // This will run the synchronize function every minute
	c.Start()
